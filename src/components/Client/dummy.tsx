...
// Define constants for Supabase URL and Storage Bucket
...
const AssignEmployeeDialog = ({ open, onOpenChange, projectId, clientId }: AssignEmployeeDialogProps) => {
  ...
  // Handle file upload for each employee
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          {employeesList.map((employee, index) => (
            <div key={index} className="border p-3 rounded-lg flex gap-4 items-center flex-wrap bg-white shadow-sm relative">
              ...
              <Input type="date" className="w-[12%]" value={employee.start_date} onChange={(e) => handleFieldChange(index, "start_date", e.target.value)} required />
              <Input type="date" className="w-[12%]" value={employee.end_date} onChange={(e) => handleFieldChange(index, "end_date", e.target.value)} required />
              <Input type="number" className="w-[8%]" value={employee.noOfDays} disabled />
              <Input type="number" className="w-[12%]" value={employee.salary} onChange={(e) => handleFieldChange(index, "salary", e.target.value)} />
              <Input type="number" className="w-[12%]" value={employee.client_billing} onChange={(e) => handleFieldChange(index, "client_billing", e.target.value)} />
              <Input type="file" className="w-[12%]" accept=".pdf,.png,.jpg" onChange={(e) => handleFileUpload(index, e)} />
