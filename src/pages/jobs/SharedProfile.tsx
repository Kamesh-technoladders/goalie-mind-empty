...
const SharedProfile = () => {
  ...
  useEffect(() => {
    // Check if share ID is valid and link hasn't expired
    const validateShare = async () => {
      setIsLoading(true);
      
      try {
        // Get expiry timestamp from URL
        ...
        // Parse data sharing options if available
        if (optionsParam) {
          try {
            ...
            // If options can't be parsed, proceed with default viewing
          }
        }
        
        // In a real app, you would validate the shareId against your database
        // This is a mock implementation that assumes all IDs with "emp" are valid
        if (shareId && shareId.includes("emp")) {
          setIsValid(true);
          // Automatically open the drawer for valid links
